...
class VPTREE : private boost::noncopyable
{
  ...
  public:
    ...
    VPTREE()
        : m_root_idx(FIRTS_NODE_IDX), m_next_idx(FIRTS_NODE_IDX), m_similar_dist(MIN_SIMILAR_DIST)
    {
    }

    VPTREE(double similar_dist)
        : m_root_idx(FIRTS_NODE_IDX), m_next_idx(FIRTS_NODE_IDX), m_similar_dist(similar_dist)
    {
    }
    ...
    void create(const Dataset::Ptr dataset, int norm_type = -1)
    {
