...
public:
    ...
    , mem_size(new size_t(0))
  {
    ...
    if (r != CUDA_SUCCESS) {
      throw std::runtime_error("Cuda device total mem error :" +
                               std::to_string(r));
    }
  ...
  const std::string getDevName() const { return device_name; }

  int getDevCount() const { return *dev_cnt; }

  size_t getDevMemory() const { return *mem_size; }
